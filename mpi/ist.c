...
int main(void)
{
  const ptrdiff_t nx = 256;
  const ptrdiff_t ny = 256;
    // MPI init
    int argc;
    char** argv;
    ...
    alloc_local = fftw_mpi_local_size_2d(nx,ny, MPI_COMM_WORLD,&local_n0,&local_n0_start);
  //other variables
 ...
 //  out array initialization
  ...
  final = fftw_alloc_real(alloc_local);

// declare fftw plans

plan_forward = fftw_mpi_plan_r2r_2d(nx, ny, in, out, MPI_COMM_WORLD,
                                     FFTW_REDFT10, FFTW_REDFT10, FFTW_ESTIMATE);

plan_backward = fftw_mpi_plan_r2r_2d(nx, ny, out, in, MPI_COMM_WORLD,
...
// simple file reader for x,y,sigma,I\n per line
    ...
    for ( i = 0; i < nx; i++ )
  {
    ...
    {
     if((j+4)<ny){
     out[i*ny+j]= out[i*ny+j] / ( nx * ny*4 );
     out[i*ny+(j+1)]= out[i*ny+(j+1)] / ( nx * ny*4 );
     out[i*ny+(j+2)]= out[i*ny+(j+2)] / ( nx * ny*4 );
     out[i*ny+(j+3)]= out[i*ny+(j+3)] / ( nx * ny*4 );
     if(out[i*ny+j]>max){
      max=out[i*ny+j];
      }
     if(out[i*ny+(j+1)]>max){
